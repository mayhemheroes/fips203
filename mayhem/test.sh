#!/usr/bin/env bash
#
# fips203/mayhem/test.sh — RUN integritychain/fips203's own test suite (`cargo test`) and emit a
# CTRF summary. exit 0 iff no test failed.
#
# PATCH-grade oracle: fips203 ships the official FIPS 203 NIST known-answer vectors
# (tests/nist_vectors — keyGen + encapDecap) and the CCTV intermediate-value vectors
# (tests/cctv_vectors), plus round-trip / negative tests (tests/integration.rs, tests/fails.rs,
# tests/native.rs). These assert BYTE-EXACT ML-KEM outputs against the standard's vectors, so a
# no-op / "exit(0)" / output-altering patch CANNOT pass. This script only RUNS the suite via
...
echo "=== running cargo test (fips203 NIST + CCTV known-answer vectors) ==="
